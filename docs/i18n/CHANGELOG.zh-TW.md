...
- SSH ZIP 檔案右鍵選單新增 **解壓縮遠端 ZIP**；遠端必須提供 `unzip`，執行前會確認覆寫風險、安全引用 POSIX shell 參數，並拒絕絕對路徑、磁碟機路徑與 `..` 上層穿越項目。
- 新增 P3 聚焦測試，涵蓋拖曳取消、跨工作區移動、避免覆寫、遠端解壓縮、終態發布，以及重新取得遠端內容的本機對遠端比對。
...
- Tree 拖曳上傳與遠端移動統一使用 POSIX 遠端路徑，並補上取消、同連線驗證、禁止移入自身、目的地碰撞檢查與 client 單次釋放。
- failed、cancelled、stopped 終態在 Tree View 與 Status Bar 顯示本地化文字。
- 檔案比對會拒絕資料夾、每次重新下載遠端內容、等待 diff 命令完成，並統一本機在左、遠端在右。

### 修正
- `privateKeyPath` 現在會讀取私鑰檔內容並傳入官方 `ssh2-sftp-client` 12.1.1 支援的 `privateKey` 選項；目錄不再被視為有效私鑰檔。
- watch 部署只有完成時清除待上傳 cache；失敗、取消與停止均保留項目供重試。
- 部署連線錯誤會同步保留到工作區檔案 log，不再只顯示短暫通知。
- JSONC hover 只顯示一份依 IDE 語系產生的欄位說明，不再疊加英文 schema 說明。
- IDE 外部執行的改名現在會同時排入舊路徑刪除與新路徑上傳，避免留下舊遠端目錄；IDE 內改名仍使用遠端 `rename`。
