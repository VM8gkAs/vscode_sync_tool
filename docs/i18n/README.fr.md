...
### Priorité d'authentification et erreurs de validation

- Pour `sftp` / `ssh`, l'authentification essaie d'abord `privateKeyPath`, puis utilise `password` en secours.
- Pour `ftp`, seule l'authentification par `password` est prise en charge.
- Messages de validation à l'exécution :
